...
const ProjectItem = ({item}:{item:ProjectItemInterface}) => {
    ...
    return (
        <li className="flex flex-col lg:flex-row gap-2 p-2 lg:p-4 transition-all sm:gap-8 md:gap-4 lg:hover:bg-slate-950 lg:hover:bg-opacity-50  rounded">
            <header className="mb-2 mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500 sm:col-span-2 lg:max-w-[20%] self-center">
            ...
            {modalDisplay && 
                (<div className="w-screen h-screen bg-slate-950 bg-opacity-50 fixed top-0 left-0 z-50 px-2 lg:py-20 lg:px-20 flex items-center justify-center flex-col gap-2" onClick={handleModal}>
                    <i className="self-end text-teal-200 hidden hover:text-cyan-500 hover:transform hover:scale-125 w-5 h-5  items-center justify-center rounded mx-5 cursor-pointer lg:flex">
                        <FontAwesomeIcon icon={faClose} onClick={handleModal}/>
                    </i>
                    <Image src={image} alt={"Preview " + name} title={"Preview " + name} className="rounded lg:w-full lg:h-full" onClick={stopPropagation} />
                </div>)
            }
        </li>
    )
