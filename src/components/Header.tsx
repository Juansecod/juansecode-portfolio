                    ...
                    <ul className="mt-5 w-max">
                        {PAGES.map(({page})=>(
                            <Link className={"group flex items-center py-3 active"} href={`#${page}`} key={page} onClick={(e)=>{setActivePage(page)}}>
                                <span className={"nav-indicator mr-4 h-px transition-all group-hover:w-16 group-hover:bg-slate-200 group-focus-visible:w-16 group-focus-visible:bg-slate-200 motion-reduce:transition-none " + (activePage.includes(page) ? "bg-slate-200 w-16" : "bg-slate-600 w-8")}></span>
